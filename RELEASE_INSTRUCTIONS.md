...
The current calchart version is 3.8.10.  In all commands below, substitute that number for `$CCVER` (meaning when you read `$CCVER`, type 3.8.10).  When incrementing, be sure to consider if it's time to bump the MINOR version.  Only do that if there's incompatibilities, such as a new feature that won't work on a previous version of CalChart.
...
```
$ git tag -a v3.8.10 -m "calchart-3.8.10"
$ git push origin v3.8.10
```
