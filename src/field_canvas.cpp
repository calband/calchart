...
FieldCanvas::~FieldCanvas(void) { ClearShapes(); }
    ...
    const CalChartConfiguration& config)
{
    ...
    if (curr_shape) {
        ...
        for (auto i = shape_list.begin(); i != shape_list.end(); ++i) {
        ...
        case CC_MOVE_ROTATE:
            ...
            if (curr_shape && (((CC_shape_1point*)curr_shape.get())->GetOrigin() != pos)) {
                ...
                mTransformer = [c1, this](const CC_coord&) {
                    ...
                    auto m = TranslationMatrix(Vector(-c1.x, -c1.y, 0)) * ZRotationMatrix(r) * TranslationMatrix(Vector(c1.x, c1.y, 0));
        ...
        case CC_MOVE_SHEAR:
            ...
            if (curr_shape && (((CC_shape_1point*)curr_shape.get())->GetOrigin() != pos)) {
                ...
                mTransformer = [o, this](const CC_coord&) {
                    ...
                    auto m = TranslationMatrix(Vector(-o.x, -o.y, 0)) * ZRotationMatrix(-ang) * YXShearMatrix(amount) * ZRotationMatrix(ang) * TranslationMatrix(Vector(o.x, o.y, 0));
                    return this->GetPoints(m);
                };
            }
            else {
            ...
            case CC_DRAG_POLY: {
                ...
                if (p != NULL) {
                    ...
                    if (std::abs(d) < polydist) {
                        d = p->y - pos.y;
                        if (std::abs(d) < polydist) {
                            mView->SelectWithLasso((CC_lasso*)curr_shape.get(),
                                event.AltDown());
                            EndDrag();
                            break;
                        }
...
void FieldCanvas::OnMouseMove(wxMouseEvent& event)
{
    ...
    if (!IsScrolling()) {
        ...
        if (mView->DoingPictureAdjustment()) {
            ...
            if ((event.Dragging() && event.LeftIsDown() && curr_shape) || (event.Moving() && curr_shape && (CC_DRAG_POLY == drag))) {
                MoveDrag(pos);
            }
        }
    }
