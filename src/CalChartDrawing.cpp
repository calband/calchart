...
std::ostream& operator<<(std::ostream& os, wxPoint point) { return os << "{" << point.x << "," << point.y << "}"; }
std::ostream& operator<<(std::ostream& os, wxSize point) { return os << "{" << point.x << "," << point.y << "}"; }
...
auto GenerateDrawArrow(CalChart::Coord::units lineLength, bool pointRight) -> std::vector<CalChart::Draw::DrawCommand>
{
    ...
    if (pointRight) {
        return std::vector<CalChart::Draw::DrawCommand>{
            CalChart::Draw::Line{
                CalChart::Coord{ -lineLength / 2, ArrowSize }, CalChart::Coord{ lineLength / 2, ArrowSize } },
            CalChart::Draw::Line{
                CalChart::Coord{ lineLength / 2 - ArrowSize, 0 }, CalChart::Coord{ lineLength / 2, ArrowSize } },
            CalChart::Draw::Line{ CalChart::Coord{ lineLength / 2 - ArrowSize, 2 * ArrowSize },
                CalChart::Coord{ lineLength / 2, ArrowSize } },
        };
    }
    return std::vector<CalChart::Draw::DrawCommand>{
        CalChart::Draw::Line{
...
static constexpr auto kHeaderLocation = std::array{ scale_t{ 0.5, 18 / kSizeY }, scale_t{ 0.5, 22 / kSizeYLandscape } };
...
void DrawCont(wxDC& dc, [[maybe_unused]] CalChart::Configuration const& config,
    CalChart::Textline_list const& print_continuity, wxRect const& bounding, bool landscape)
{
    ...
    auto numLines
        = std::count_if(print_continuity.begin(), print_continuity.end(), [](auto&& i) { return i.on_sheet; });
    ...
    for (auto& cont : print_continuity) {
        ...
        if (cont.center) {
            ...
            for (auto& chunk : chunks) {
                std::visit(CalChart::overloaded{
                               [&dc, &x, contPlainFont, contBoldFont, contItalFont, contBoldItalFont](
                                   [[maybe_unused]] CalChart::TextChunk const& c) {
                                   wxString text = wxString::FromUTF8(c.text.c_str(), c.text.length());
                                   switch (c.font) {
        ...
        for (auto& chunk : chunks) {
            std::visit(
                CalChart::overloaded{
                    [&dc, &x, y, contPlainFont, contBoldFont, contItalFont, contBoldItalFont](
                        CalChart::TextChunk const& c) {
                        switch (c.font) {
                        case CalChart::PSFONT::NORM:
                        case CalChart::PSFONT::SYMBOL:
                            dc.SetFont(contPlainFont);
...
static std::pair<CalChart::Coord, CalChart::Coord> GetMarcherBoundingBox(std::vector<CalChart::Point> const& pts)
{
...
auto CalculatePointsPerLine(
    CalChart::PrintContinuityLayout::VStack const& print_continuity, wxRect const& bounding, int linePad)
{
    auto numLines = std::count_if(
        print_continuity.lines.begin(), print_continuity.lines.end(), [](auto&& i) { return i.on_sheet; });
...
namespace {
    ...
    auto GenerateLargePrintElements(bool landscape, wxSize page, std::string sheetName)
    {
        ...
        CalChart::append(drawCmds, CalChart::Draw::toCenterText(kHeader) + scaleSize(page, kHeaderLocation[landscape]));
        CalChart::append(
            drawCmds, CalChart::Draw::toCenterText(sheetName) + scaleSize(page, kNumberPosition[landscape][0]));
        CalChart::append(
            drawCmds, CalChart::Draw::toCenterText(sheetName) + scaleSize(page, kNumberPosition[landscape][1]));
        return drawCmds;
    }
    ...
    auto GeneratePrintElements(bool landscape, wxSize page) -> std::vector<CalChart::Draw::DrawCommand>
    {
        ...
        CalChart::append(drawCmds,
            std::vector<CalChart::Draw::DrawCommand>{ CalChart::Draw::Rectangle(
                scaleSize(page, kLowerNumberBox[landscape][0]), scaleSize(page, kLowerNumberBox[landscape][1])) });
        CalChart::append(drawCmds,
            CalChart::Draw::toDrawCommands(kTextLabels[landscape] | std::views::transform([page](auto&& labelData) {
                auto [label, scale, lineLength] = labelData;
                return GenerateDrawLineOverText(label, page.x * lineLength) + scaleSize(page, scale);
            }) | std::views::join));

        CalChart::append(drawCmds,
            CalChart::Draw::toDrawCommands(kLabels[landscape] | std::views::transform([page](auto&& labelData) {
                auto [label, scale] = labelData;
                return std::vector{ CalChart::Draw::toCenterText(label) + scaleSize(page, scale) };
            }) | std::views::join));

        CalChart::append(
            drawCmds, CalChart::Draw::toDrawCommands(kArrows[landscape] | std::views::transform([page](auto&& arrow) {
    ...
    wxDC& dc, wxSize pageSize, CalChart::Configuration const& config, CalChart::Sheet const& sheet, bool landscape)
{
    ...
    auto printDrawCommands = GenerateDrawCommands(dc, config, printLayout,
        wxRect(wxPoint(10, pageSize.y * kContinuityStart[landscape]),
            wxSize(pageSize.x - 20, pageSize.y - pageSize.y * kContinuityStart[landscape])),
        landscape);
    ...
    wxCalChart::Draw::DrawCommandList(dc,
        wxCalChart::Draw::DrawSurface{
            { 0, 0 }, wxSize(pageSize.x - 20, pageSize.y * (1 - kContinuityStart[landscape])) },
...
void DrawForPrintingElements(wxDC& dc, wxSize pageSize, CalChart::Sheet const& sheet, bool landscape)
{
    ...
    CalChart::append(drawCmds,
        CalChart::Draw::withFont(CalChart::Font{ 16, CalChart::Font::Family::Roman, CalChart::Font::Style::Normal,
                                     CalChart::Font::Weight::Bold },
            GenerateLargePrintElements(landscape, pageSize, sheet.GetPrintNumber())));

    CalChart::append(
        drawCmds, CalChart::Draw::withFont(CalChart::Font{ 8 }, GeneratePrintElements(landscape, pageSize)));
    ...
    -> CalChart::Draw::DrawCommand
{
    ...
    auto fontSize
        = std::min<int>(CalculatePointsPerLine(printLayout, bounding, linePad), config.Get_PrintContMaxFontSize());
    ...
    try {
        return CalChart::Draw::withBrushAndPen(wxCalChart::toBrushAndPen(*wxBLACK_PEN),
            CalChart::Draw::withFont(
                context.plain, CalChart::PrintContinuityLayout::ToDrawCommand(printLayout, context)));
...
wxString DecodeString(std::string const& text)
{
    const char* data = text.data();
    size_t len = text.size();

    // Try strict UTF-8 first
    wxString result = wxString::FromUTF8(data, len);
    if (!result.empty() || len == 0)
        return result;

    // Fall back to Windows-1252 (common on Windows-authored files)
    wxCSConv cp1252(wxFONTENCODING_CP1252);
    result = wxString(data, cp1252, len);
    if (!result.empty())
        return result;

    // Last resort: Mac Roman (common on old Mac-authored files)
    wxCSConv macRoman(wxFONTENCODING_MACROMAN);
    return wxString(data, macRoman, len);
}

// this draws the yardline labels
static void DrawText(
    wxDC& dc, std::string const& text, wxPoint where, CalChart::Draw::Text::TextAnchor anchor, bool withBox)
{
    wxString wxtext = DecodeString(text);
    auto textSize = dc.GetTextExtent(wxtext);
    using TextAnchor = CalChart::Draw::Text::TextAnchor;
    ...
    dc.DrawText(wxtext, where);
}

// The CalChart::Draw::DrawCommand is a data structure that dictates "what to draw", but not how or where specficially.
// The How is interpreted by the wxWidget's Device Context logic -- effectively we translate the
// `CalChart::Draw::DrawCommand` type to the appropriate `wxDC` API call with the appropriate offsets from the
// `CalChart::Draw::DrawCommand` objects. But Layout becomes a bit more trickly we have `CalChart::Draw::DrawStack`
// objects.  Stacking things stacking things together, especially Text, becomes complicated because the size of the Text
// due to the font choice can change, the "where" to draw becomes non-trivial.
//
...
namespace details {
    ...
    auto operator+(wxPoint a, CalChart::Coord b) { return wxPoint{ a.x + b.x, a.y + b.y }; }
    ...
    {
        std::visit([&dc, surface](auto&& c) { DrawCommand(dc, surface, c); }, cmd);
    }
}
}
