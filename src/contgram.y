...
#pragma warning(disable : 4065) //Disable the "switch statement contains 'default' but no 'case' labels" warning that is flagged by the code generated by Bison
    ...
    % token rwP rwNP rwR rwREM rwSP rwDOF rwDOH % token pBLAM pCOUNTERMARCH pDMCM pDMHS pEVEN pEWNS pFOUNTAIN pFM pFMTO % token pGRID pHSDM pHSCM pMAGIC pMARCH pMT pMTRM pNSEW pROTATE % token fDIR fDIRFROM fDIST fDISTFROM fEITHER fOPP fSTEP % token UNKNOWN_TOKEN

    % { /* Constants and variables */
        %
      }
    % token<f> FLOATCONST % token<v> VARIABLE % token<d> DEFINECONST
