...
private:
	CCAppServer(CalChartApp* app) : mApp(app) {}
...
private:
	CCAppClient() : mConnection(nullptr) {}
...
// Connection pair:
// Servers will get Poked, then they turn around and ask the server object to open it.
...
public:
	CCAppServerConnection(CCAppServer* server) : mServer(server) {}
...
class CCAppClientConnection : public wxConnection
{
	using super = wxConnection;
public:
	CCAppClientConnection(CCAppClient* client) : mClient(client) {}
...
CCAppServer::~CCAppServer()
{
	...
	{
		// this should cause the connection to turn around and call DestroyConnection()
