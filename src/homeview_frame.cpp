...
public:
    MyPrintout(const wxString& title, const CC_show& show)
        : wxPrintout(title)
        , mShow(show)
    {
    }
...
HomeViewFrame::HomeViewFrame(wxDocument* doc, wxView* view,
    wxDocParentFrame* frame, const wxPoint& pos,
    const wxSize& size)
    : wxDocChildFrame(doc, view, frame, -1, wxT("CalChart"), pos, size)
{
    ...
    anim_menu->Append(CALCHART__EDIT_CONTINUITY,
        wxT("&Edit Continuity...\tCTRL-E"),
        wxT("Edit continuity for this stuntsheet"));
    anim_menu->Append(CALCHART__ANIMATE, wxT("&Animate...\tCTRL-RETURN"),
        wxT("Open animation window"));
    anim_menu->Append(CALCHART__OMNIVIEW, wxT("&OmniView..."),
        wxT("Open CalChart Omniviewer"));
...
void HomeViewFrame::OnCmdPrint(wxCommandEvent& event)
{
    ...
    MyPrintout printout(wxT("My Printout"),
        *static_cast<CC_show*>(GetDocument()));
...
{
    // grab our current page setup.
    wxPrintPreview* preview = new wxPrintPreview(new MyPrintout(wxT("My Printout"),
                                                     *static_cast<CC_show*>(GetDocument())),
        new MyPrintout(wxT("My Printout"),
                                                     *static_cast<CC_show*>(GetDocument())),
        gPrintDialogData);
    if (!preview->Ok()) {
        delete preview;
...
{
    PrintPostScriptDialog dialog(static_cast<CC_show*>(GetDocument()), false,
        this);
...
{
    PrintPostScriptDialog dialog(static_cast<CC_show*>(GetDocument()), true,
        this);
    if (dialog.ShowModal() == wxID_OK) {
        dialog.PrintShow();
    }
...
void HomeViewFrame::OnCmdAbout(wxCommandEvent& event) { TopFrame::About(); }

void HomeViewFrame::OnCmdHelp(wxCommandEvent& event) { TopFrame::Help(); }
