...
BasicCalChartCommand::BasicCalChartCommand(CalChartDoc& doc,
...
SetDescriptionCommand::SetDescriptionCommand(CalChartDoc& show,
    const wxString& newdescr)
...
SetShowInfoCommand::SetShowInfoCommand(CalChartDoc& show, unsigned numPoints,
    unsigned numColumns,
...
void SetSheetCommand::DoAction() { mDoc.SetCurrentSheet(mSheetNum); }

// SetSheetTitleCommand
// Set the description of the current sheet
SetSheetTitleCommand::SetSheetTitleCommand(CalChartDoc& show,
...
SetSheetBeatsCommand::SetSheetBeatsCommand(CalChartDoc& show,
...
AddSheetsCommand::AddSheetsCommand(CalChartDoc& show,
...
AddSheetsOtherShowCommand::AddSheetsOtherShowCommand(
    CalChartDoc& show, const CC_show::CC_sheet_container_t& sheets,
...
// were
// when command was created.
SetSheetAndSelectCommand::SetSheetAndSelectCommand(CalChartDoc& doc,
    const wxString cmdName)
...
ImportPrintContinuityCommand::ImportPrintContinuityCommand(
    CalChartDoc& show, const std::vector<std::string>& print_cont)
...
SetPrintContinuityCommand::SetPrintContinuityCommand(
    CalChartDoc& show, unsigned sheet, const std::string& number,
...
MovePointsOnSheetCommand::MovePointsOnSheetCommand(CalChartDoc& show,
    unsigned ref)
...
RotatePointPositionsCommand::RotatePointPositionsCommand(CalChartDoc& show,
    unsigned rotateAmount,
    unsigned ref)
    : super(show, ref)
{
    ...
    std::transform(mPoints.begin(), mPoints.end(),
        std::back_inserter(finalPositions),
        [=](unsigned i) { return sheet->GetPosition(i, mRef); });
...
MovePointsCommand::MovePointsCommand(
    CalChartDoc& show, const std::map<unsigned, CC_coord>& newPosition,
    unsigned ref)
    : super(show, ref)
{
...
SetReferencePointToRef0::SetReferencePointToRef0(CalChartDoc& show,
    unsigned ref)
...
void SetSymbolCommand::DoAction()
{
    SetSheetAndSelectCommand::DoAction(); // sets selected and page
    CC_show::CC_sheet_iterator_t sheet = mDoc.GetCurrentSheet();
    // possible optimization:  Don't save as map, since they all get moved to new
    // symbol...
...
SetContinuityTextCommand::SetContinuityTextCommand(CalChartDoc& show,
    SYMBOL_TYPE which,
    const wxString& text)
...
{
    SetSheetAndSelectCommand::DoAction(); // sets selected and page
    mDoc.GetCurrentSheet()->SetContinuityText(mWhichCont,
        mContinuity.ToStdString());
}
...
void SetLabelVisibilityCommand::DoAction()
{
    SetSheetAndSelectCommand::DoAction(); // sets selected and page
    CC_show::CC_sheet_iterator_t sheet = mDoc.GetCurrentSheet();
    for (auto iterator = mLabelVisibility.begin();
         iterator != mLabelVisibility.end(); iterator++) {
        sheet->GetPoint(iterator->first).SetLabelVisibility(iterator->second);
    }
}

SetLabelVisibleCommand::SetLabelVisibleCommand(CalChartDoc& show,
    bool isVisible)
    : super(show)
{
