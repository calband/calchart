...
bool HomeViewView::OnCreate(wxDocument* doc, long WXUNUSED(flags))
{
    mShow = static_cast<CC_show*>(doc);
    mFrame = new FieldFrame(
        doc, this, wxStaticCast(wxGetApp().GetTopWindow(), wxDocParentFrame),
        wxPoint(50, 50), wxSize(GetConfiguration_FieldFrameWidth(),
                             GetConfiguration_FieldFrameHeight()));
...
void HomeViewView::OnDraw(wxDC* dc) {}

void HomeViewView::OnUpdate(wxView* WXUNUSED(sender), wxObject* hint)
{
...
bool HomeViewView::OnClose(bool deleteWindow)
{
...
void HomeViewView::OnWizardSetup(CC_show& show)
{
...
int HomeViewView::FindPoint(CC_coord pos) const
{
    return mShow->GetCurrentSheet()->FindPoint(pos.x, pos.y, 0);
}

CC_coord HomeViewView::PointPosition(int which) const
{
    return mShow->GetCurrentSheet()->GetPosition(which, 0);
}

void HomeViewView::GoToSheet(size_t which)
{
...
void HomeViewView::GoToNextSheet()
{
    GoToSheet(mShow->GetCurrentSheetNum() + 1);
}

void HomeViewView::GoToPrevSheet()
{
    GoToSheet(mShow->GetCurrentSheetNum() - 1);
}
...
void HomeViewView::SelectWithLasso(const CC_lasso* lasso, bool toggleSelected)
{
    mShow->SelectWithLasso(*lasso, toggleSelected, 0);
}

// Select points within rectangle
void HomeViewView::SelectPointsInRect(const CC_coord& c1, const CC_coord& c2,
    bool toggleSelected)
{
    CC_lasso lasso(c1);
