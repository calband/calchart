...
void HelpDialog::LoadTopic(const std::string& topicId)
{
    ...
    // Get the HTML content from HelpManager
    std::string html = mHelpManager.GetHelpTopicAsHtml(topicId);

    if (html.empty()) {
        mWebView->SetPage("<html><body><p>Help topic not found.</p></body></html>", "");
    } else {
        mWebView->SetPage(wxString::FromUTF8(html), "");
    }
...
void HelpDialog::OnBack(wxCommandEvent&)
{
    ...
    mCurrentTopicId = mBackStack.top();
    mBackStack.pop();

    mWebView->SetPage(wxString::FromUTF8(mHelpManager.GetHelpTopicAsHtml(mCurrentTopicId)), "");
...
void HelpDialog::OnForward(wxCommandEvent&)
{
    ...
    mCurrentTopicId = mForwardStack.top();
    mForwardStack.pop();
