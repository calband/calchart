...
static auto trucate(CalChart::Coord c) { return (c / 5) * 5; }
...
TEST_CASE("CalChartShapeTests", "Shape_crosshairs")
{
    ...
    CHECK((uut.GetCC_DrawCommand() + CalChart::Coord{ 20, 20 })
...
TEST_CASE("Shape_PolyPoint", "CalChartShapeTests")
{
    ...
    CHECK(uut.GetPointsOnLine(8)
        == std::vector<CalChart::Coord>{
            { 10, 10 }, { 25, 25 }, { 41, 39 }, { 44, 49 }, { 22, 49 }, { 17, 43 }, { 32, 28 }, { 47, 13 } });
    result = uut.GetPointsOnLine(16);
    CHECK(uut.GetPointsOnLine(16)
        == std::vector<CalChart::Coord>{ { 10, 10 }, { 17, 17 }, { 24, 24 }, { 31, 30 }, { 38, 37 }, { 45, 43 },
            { 50, 49 }, { 40, 49 }, { 30, 49 }, { 20, 49 }, { 10, 49 }, { 17, 42 }, { 24, 35 }, { 31, 29 }, { 38, 22 },
            { 45, 16 } });
}
...
TEST_CASE("Shape_PolyPoint3", "CalChartShapeTests")
{
    ...
    CHECK(uut.GetPointsOnLine(5)
        == std::vector<CalChart::Coord>{ { 0, 0 }, { 32, 0 }, { 64, 0 }, { 96, 0 }, { 128, 0 } });
}
...
TEST_CASE("Test_Inside", "CalChartShapeTests")
{
    ...
    CHECK(!CalChart::Inside(
        { 5, 5 }, CalChart::RawPolygon_t{ CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 } }));
    CHECK(CalChart::Inside({ 5, 5 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 10, 5 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 0, 5 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 5, 0 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 1, 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 1, 5 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 1, 9 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 5, 9 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 9, 9 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 9, 5 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 9, 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 5, 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 9, 1 } }));
    // a more unusual shape with inner blank spots.  Using math here to make it clear where we are going into boundaries
    CHECK(!CalChart::Inside({ 5, 5 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 1 + 1, 1 + 2 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 1 + 2, 1 + 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 9 - 1, 9 - 2 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 9 - 2, 9 - 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 1 + 1, 9 - 2 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 1 + 2, 9 - 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(!CalChart::Inside({ 9 - 1, 1 + 2 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));
    CHECK(CalChart::Inside({ 9 - 2, 1 + 1 },
        CalChart::RawPolygon_t{
            CalChart::Coord{ 1, 1 }, CalChart::Coord{ 9, 9 }, CalChart::Coord{ 1, 9 }, CalChart::Coord{ 9, 1 } }));

    // a star pattern
    CHECK(!CalChart::Inside({ 5, 5 },
        CalChart::RawPolygon_t{ CalChart::Coord{ 1, 1 }, CalChart::Coord{ 5, 9 }, CalChart::Coord{ 9, 1 },
            CalChart::Coord{ 1, 6 }, CalChart::Coord{ 1, 8 }, CalChart::Coord{ 9, 8 }, CalChart::Coord{ 9, 6 } }));
    CHECK(CalChart::Inside({ 2, 7 },
        CalChart::RawPolygon_t{ CalChart::Coord{ 1, 1 }, CalChart::Coord{ 5, 9 }, CalChart::Coord{ 9, 1 },
            CalChart::Coord{ 1, 6 }, CalChart::Coord{ 1, 8 }, CalChart::Coord{ 9, 8 }, CalChart::Coord{ 9, 6 } }));
    CHECK(CalChart::Inside({ 8, 7 },
        CalChart::RawPolygon_t{ CalChart::Coord{ 1, 1 }, CalChart::Coord{ 5, 9 }, CalChart::Coord{ 9, 1 },
            CalChart::Coord{ 1, 6 }, CalChart::Coord{ 1, 8 }, CalChart::Coord{ 9, 8 }, CalChart::Coord{ 9, 6 } }));
}
...
{
    auto controlPoints = std::vector<CalChart::Coord>{ { 10, 10 }, { 500, 400 }, { 10, 400 }, { 500, 10 } };
    ...
    //                 std::visit(
    //                     CalChart::overloaded{
    //                         [](CalChart::Draw::Line line) { std::cout << "CalChart::Draw::Line{ CalChart::Coord{ " <<
    //                         line.c1.x << ", " << line.c1.y << "}, CalChart::Coord{ " << line.c2.x << ", " <<
    //                         line.c2.y << "} },\n"; },
    //                         [](auto&&) {},
    //                     },
    //                     item);
...
{
    {
        auto uut = CalChart::Curve(std::vector<CalChart::Coord>{ { 10, 10 }, { 500, 400 }, { 10, 400 }, { 500, 10 } });
...
TEST_CASE("Shape_Curve_JSONSerializeDeserialize", "CalChartShapeTests")
{
    {
        auto uut = CalChart::Curve(std::vector<CalChart::Coord>{ { 10, 10 }, { 500, 400 }, { 10, 400 }, { 500, 10 } });
